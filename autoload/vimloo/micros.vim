...
command! -nargs=1 -complete=file LOGON call class#viml#loger#SetLogFile(<f-args>)
command! -nargs=0 LOGOFF call class#viml#loger#SetLogFile('')
...
command! -nargs=+ LOG call class#viml#loger#hLog(eval(<q-args>))
command! -nargs=+ ELOG call class#viml#loger#hLog('-ErrorMsg ' . eval(<q-args>))
command! -nargs=+ WLOG call class#viml#loger#hLog('-WarningMsg ' . eval(<q-args>))
command! -nargs=+ -complete=highlight SLOG call class#viml#loger#hLog(<q-args>)

command! -nargs=+ DLOG
        \ if exists('g:DEBUG') <bar>
        \     call class#viml#loger#hLog('-DEBUG ' . eval(<q-args>)) <bar>
        \ endif
