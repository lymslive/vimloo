...
let s:Bitwise = package#import('jp#Bitwise')
...
function! s:new(...) abort
  ...
  let seed = 2 <= a:0 ? a:2 : s:make_seed()
...
function! s:_get_generator(arg) abort
    " todo: fix to select generator randomly in Random/ subdir
  let arg =
  \ empty(a:arg)
  \ ? 'Xor128'
  \ : a:arg
  if type(arg) == type('')
    if !has_key(s:loaded_generator_modules, arg)
      let module_name = 'jp#Random#' . arg
      let module = package#import(module_name)
