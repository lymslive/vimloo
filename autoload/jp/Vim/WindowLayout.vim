...
" load: 
function! jp#Vim#WindowLayout#load() abort "{{{
  let s:BM = package#import('jp#Vim#BufferManager')
  let s:L =  package#import('jp#Data#List')

  for layout_manager in glob(s:thisdir . '/WindowLayout/*Layout.vim', '', 1)
    " gather some kind of layout engines, located under Vim.WindowLayout namespace
    let name = matchstr(layout_manager, '\CVim/WindowLayout/\zs\w\+\zeLayout\.vim$')
    let s:layout.__layouts[tolower(name)] = package#rimport(s:thisdir, layout_manager).new()
  endfor
endfunction "}}}
