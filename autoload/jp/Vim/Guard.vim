...
let s:List = package#import('jp#Data#List')
let s:Dict = package#import('jp#Data#Dict')

" load: 
function! jp#Vim#Guard#load() abort "{{{
...
function! jp#Vim#Guard#export() abort "{{{
  let s:EXPORT = copy(s:const)
  let s:EXPORT['<SID>'] = ''
  return s:EXPORT
endfunction "}}}
